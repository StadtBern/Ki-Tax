...
public class AbwesenheitAbschnittRuleTest {
	...
	public void testAbschnitteLongAbwesenheit() {
		...
		checkAbwesenheitAbschnitte(betreuung, abwesenheit, zeitabschnitte);
	}
	...
	public void testAbschnitteSeveralLongAbw() {
		...
		Assert.assertNotNull(zeitabschnitte);
		Assert.assertEquals("Trotz 2 Abwesenheiten nur die erste wird beruecksichtigt", 2, zeitabschnitte.size());

		checkAbwesenheitAbschnitte(betreuung, abwesenheit1, zeitabschnitte);
	}
	...
	public void testAbschnitteShortLongAbw() {
		...
		Assert.assertNotNull(zeitabschnitte);
		Assert.assertEquals("Die erste Abwesenheit wird nicht beruecksichtigt da sie kurz ist. Nur die 2 erstellt die Zeitabschnitte", 2, zeitabschnitte.size());

		checkAbwesenheitAbschnitte(betreuung, lateAbwesenheit, zeitabschnitte);
	}

	private void checkAbwesenheitAbschnitte(Betreuung betreuung, AbwesenheitContainer abwesenheit, List<VerfuegungZeitabschnitt> zeitabschnitte) {
		Assert.assertFalse(zeitabschnitte.get(0).isLongAbwesenheit());
		Assert.assertEquals(betreuung.extractGesuchsperiode().getGueltigkeit().getGueltigAb(), zeitabschnitte.get(0).getGueltigkeit().getGueltigAb());
		Assert.assertEquals(abwesenheit.getAbwesenheitJA().getGueltigkeit().getGueltigAb().plusDays(Constants.ABWESENHEIT_DAYS_LIMIT - 1), zeitabschnitte.get(0).getGueltigkeit().getGueltigBis());

		Assert.assertTrue(zeitabschnitte.get(1).isLongAbwesenheit());
		Assert.assertEquals(abwesenheit.getAbwesenheitJA().getGueltigkeit().getGueltigAb().plusDays(Constants.ABWESENHEIT_DAYS_LIMIT), zeitabschnitte.get(1).getGueltigkeit().getGueltigAb());
		Assert.assertEquals(betreuung.extractGesuchsperiode().getGueltigkeit().getGueltigBis(), zeitabschnitte.get(1).getGueltigkeit().getGueltigBis());
	}
}
