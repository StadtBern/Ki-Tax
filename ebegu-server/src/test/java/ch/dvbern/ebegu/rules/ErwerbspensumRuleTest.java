...
public class ErwerbspensumRuleTest {
	...
	private List<VerfuegungZeitabschnitt> calculateRules(Betreuung betreuung) {
	...
	public void testKeinErwerbspensum() {
		...
		List<VerfuegungZeitabschnitt> result = calculateRules(betreuung);
	...
	public void testRoundToTens() throws Exception {
		...
		List<VerfuegungZeitabschnitt> result2 = calculateRules(betreuung);
		Assert.assertEquals(0, result2.get(0).getAnspruchberechtigtesPensum());

		gesuch.getGesuchsteller1().setErwerbspensenContainers(new HashSet<>());
		gesuch.getGesuchsteller1().addErwerbspensumContainer(TestDataUtil.createErwerbspensum(START_PERIODE, ENDE_PERIODE, 1, 0));
		List<VerfuegungZeitabschnitt> result3 = calculateRules(betreuung);
		...
		List<VerfuegungZeitabschnitt> result4 = calculateRules(betreuung);
		...
		List<VerfuegungZeitabschnitt> result5 = calculateRules(betreuung);
		...
		List<VerfuegungZeitabschnitt> result6 = calculateRules(betreuung);
		...
		List<VerfuegungZeitabschnitt> result7 = calculateRules(betreuung);
		...
		List<VerfuegungZeitabschnitt> result8 = calculateRules(betreuung);
		...
		List<VerfuegungZeitabschnitt> result9 = calculateRules(betreuung);
		...
		List<VerfuegungZeitabschnitt> result10 = calculateRules(betreuung);
		Assert.assertEquals(100, result10.get(0).getAnspruchberechtigtesPensum());

		gesuch.getGesuchsteller1().setErwerbspensenContainers(new HashSet<>());
		gesuch.getGesuchsteller1().addErwerbspensumContainer(TestDataUtil.createErwerbspensum(START_PERIODE, ENDE_PERIODE, 100, 0));
		List<VerfuegungZeitabschnitt> result11 = calculateRules(betreuung);
		...
		List<VerfuegungZeitabschnitt> result12 = calculateRules(betreuung);
