...
public class VerfuegungZeitabschnitt extends AbstractDateRangedEntity implements Comparable<VerfuegungZeitabschnitt> {
	...
	@Transient
	private BigDecimal massgebendesEinkommenNachAbzugFamgr = ZERO;
	...
	public BigDecimal getMassgebendesEinkommen() {
		...
		return MathUtil.GANZZAHL.subtract(massgebendesEinkommenVorAbzugFamgr, this.abzugFamGroesse == null ? BigDecimal.ZERO : this.abzugFamGroesse);
	}
	...
	public void addBemerkung(String bem) {
		...
		this.bemerkungen = Joiner.on("\n").skipNulls().join(StringUtils.defaultIfBlank(this.bemerkungen, null), StringUtils.defaultIfBlank(bem, null));
	}
	 ...
	 * Betreuungspensum. Falls das anspruchsberechtigte Pensum unter dem Betreuungspensum liegt, entspricht das
	 * BG-Pensum dem anspruchsberechtigten Pensum.
	 * <p>
	 * Ein Kind mit einem Betreuungspensum von 60% und einem anspruchsberechtigten Pensum von 40% hat ein BG-Pensum von
	 * 40%. Ein Kind mit einem Betreuungspensum von 40% und einem anspruchsberechtigten Pensum von 60% hat ein BG-Pensum
	 * von 40%.
	 */
	...
	public String toString() {
		...
		sb.append("[").append(Constants.DATE_FORMATTER.format(getGueltigkeit().getGueltigAb())).append(" - ")
				.append(Constants.DATE_FORMATTER.format(getGueltigkeit().getGueltigBis())).append("] ").append(" EP GS1: ").append(erwerbspensumGS1).append("\t")
				.append(" EP GS2: ").append(erwerbspensumGS2).append("\t").append(" BetrPensum: ").append(betreuungspensum).append("\t").append(" Anspruch: ")
				.append(anspruchberechtigtesPensum).append("\t").append(" BG-Pensum: ").append(getBgPensum()).append("\t").append(" Vollkosten: ").append(vollkosten).append("\t")
				.append(" Elternbeitrag: ").append(elternbeitrag).append("\t").append(" Bemerkungen: ").append(bemerkungen).append("\t").append(" Einkommen: ")
				.append(massgebendesEinkommenVorAbzugFamgr).append("\t").append(" Abzug Fam: ").append(abzugFamGroesse);
		return sb.toString();
	}
	...
	public boolean isSame(VerfuegungZeitabschnitt that) {
		...
		return erwerbspensumGS1 == that.erwerbspensumGS1 && erwerbspensumGS2 == that.erwerbspensumGS2 && betreuungspensum == that.betreuungspensum
				&& fachstellenpensum == that.fachstellenpensum && anspruchspensumRest == that.anspruchspensumRest && anspruchberechtigtesPensum == that.anspruchberechtigtesPensum
				&& Objects.equals(abzugFamGroesse, that.abzugFamGroesse) && Objects.equals(massgebendesEinkommenVorAbzugFamgr, that.massgebendesEinkommenVorAbzugFamgr)
				&& zuSpaetEingereicht == that.zuSpaetEingereicht && wohnsitzNichtInGemeindeGS1 == that.wohnsitzNichtInGemeindeGS1
				&& wohnsitzNichtInGemeindeGS2 == that.wohnsitzNichtInGemeindeGS2 && bezahltVollkosten == that.bezahltVollkosten
	...
	public int compareTo(VerfuegungZeitabschnitt other) {
		...
		compareToBuilder.append(this.getId(), other.getId()); // wenn ids nicht gleich sind wollen wir auch compare to
																// nicht gleich
	...
	public BigDecimal getMassgebendesEinkommenNachAbzugFamgr() {
		...
		return massgebendesEinkommenNachAbzugFamgr;
	}

	public void setMassgebendesEinkommenNachAbzugFamgr(BigDecimal massgebendesEinkommenNachAbzugFamgr) {
